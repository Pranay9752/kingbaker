...
const ProductMetaDetail = ({ details }) => {
  ...
  if (details?.[0]?.key) {
    return (
      <>        
        <Element name="productMetaSection"  id="productMetaSection" className="grid grid-cols-3 py-2 gap-2 md:hidden">
          <style>
            {`
            .editor ol {
              list-style-type: disc; /* Use bullets for ordered lists */
        ...
        <Element name="productMetaSection"  id="productMetaSection" className="hidden md:flex flex-col gap-3 text-left">
          {details?.map((item, index) => {
            return (
              <div key={index}>
                <h4 className="text-xl text-gray-800 font-semibold">
            ...
            <div key={index}>
              ...
              {Object.keys(item?.detail ?? {}).map((key, i) => (
                <div key={i}>
                  ...
                  {Array.isArray((item?.detail ?? {})?.[key]) ? (
                    <ul className="list-disc list-inside text-gray-600 flex flex-col gap-2 m-3 mt-2">
                      {(item?.detail ?? {})?.[key].map((itemValue, j) => (
                        <li key={j}>
                          <span>{itemValue}</span>
                        </li>
                      ))}
                    ...
                    <ul className="list-disc list-inside text-gray-600 flex flex-col gap-1 m-3 mt-2 text-sm">
                      {Object.keys((item?.detail ?? {})?.[key]).map(
                        (itemKey, j) => (
                          <li key={j}>
                            {itemKey} - {(item?.detail ?? {})?.[key]?.[itemKey]}
                          </li>
                        )
                      )}
                    </ul>
