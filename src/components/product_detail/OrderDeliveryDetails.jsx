...
import { deleteAddon, deleteOrder, updateAddonQuantity, updateShipping } from "../../redux/slices/ecom/orderSlice";
...
const OrderDeliveryDetails = ({
    ...
    const handleSelectSlot = ({ delivery, slot }) => {        
        ...
        dispatch(updateShipping({ index, name:"fee", value: delivery?.price ??0 }))
        dispatch(updateShipping({ index, name:"method", value: delivery?.title ?? "" }))
    };
    ...
    return (
        <>
            ...
            >
                <div className={`bg-white text-black rounded-lg  ${dense ? "" : "p-4"}`}>
                    ...
                    {addons.map((addon) => (
                        <div key={addon.id} className={`flex items-center mb-2 border-b ${dense ? "pb-2" : "pb-3"}`}>
                            <img
                                src={addon.image}
                                alt={addon.name}
                ...
                <div className={`w-full px-4 flex flex-col gap-2 ${isCart && 'hidden'}`}>
                    {openAddAddress ? (
                ...
                <div className={`bg-white text-black rounded-lg  ${dense ? "px-4 m-1  border py-2" : "p-4 mb-4"}`}>
                    <div className="flex justify-between items-center">
                        <div>
                            <p className="flex items-center gap-2">
                                ...
                                Delivery on{" "}
                                <span className="font-medium">{isValid(new Date(deliveryDetails.date)) ? format(new Date(deliveryDetails.date), 'dd MMM') : deliveryDetails.date}</span>
                            </p>
                            <p className="font-medium">{deliveryDetails?.method ?? "Standard Delivery"}</p>
                            <p className="font-medium">
                                {deliveryDetails.timeSlot}{" "}
                                <span className="text-orange-500 font-light ml-2">₹ {deliveryDetails?.fee ?? 0}</span>
                            </p>
                        </div>
                        <button type={"button"} onClick={() => setDateModal(true)} className="ml-auto text-gray-600 border rounded border-gray-600 hover:bg-gray-300 text-sm px-1 py-0.5">
                            Change
                        </button>
                    </div>
                </div>
                ...
                <div className={`space-y-2 px-4 md:hidden ${isCart && 'hidden'}`}>
                    ...
                    <div className={`w-full text-lg border-gray-500 bg-slate-100 text-black py-2 rounded-lg flex justify-start items-center px-4 ${isCart && 'hidden'}`}>
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24"
                            fill="currentColor"
