            ...
            <div className="w-full  p-4 rounded-lg overflow-y-auto h-[70vh]">
              {orderDetailIndex !== null ? (
                ...
                isPrintChallan={false}
                  ...
                  order={transformOrderData({...data?.data?.delivery_details?.[orderDetailIndex]?.mainItem,addOn:data?.data?.delivery_details?.[orderDetailIndex]?.addOn}) ?? {}}
                //   darkMode
                />
              ) : (
                // <OrderDetailsCard order={orders[orderDetailIndex]} />
                data?.data?.delivery_details?.map((order, index) => {
                    return (
                  <UserOrderCard
