...
function UpdateOrderModal({ data, onClose }) {
  ...
  const onSubmit = async (value) => {
    ...
    const orderData = {
      addOn: addons,
      delivery_date: new Date(value.day),
    };
  ...
  const addAddons = () => {setShowAddonModal(true) };
  const getSelectedAddons =(addOns)=>{
    setAddons((prev) =>[...prev,...addOns])
  ...
  return (
    <>
      ...
      >
        <AddAddonModal getSelectedAddons={getSelectedAddons} onClose={() => setShowAddonModal(false)} />
      </ModalWrapper>
    </>
  );
}
