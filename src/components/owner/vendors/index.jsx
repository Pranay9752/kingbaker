...
const OwnerVendors = ({ }) => {
  ...
  const toggleVendorMapping = (vendor) => {
    console.log(vendor)
    setVendorMapping(vendor || null)
  }
  ...
  return (
    <>
          ...
          vendorMapping ? <VendorMapping vendor_id={vendorMapping?._id} onBack={toggleVendorMapping}/> :
            <div className="w-full bg-black text-gray-300 p-4 rounded-lg">
              <div className="flex justify-between items-center gap-3 mb-6">
                ...
                <BasicButton
                  onClick={() => setSelectedVendor(-1)}
                  className={`text-xs md:text-base border-2 rounded-xl px-2 py-2 whitespace-nowrap border-[#1a1f25] focus:scale-95 hover:bg-[#1a1f25]`}
                >
                  Add Vendor
                </BasicButton>
              </div>
                    ...
                    {filteredVendors.length > 0
                      ? filteredVendors.map((vendor, index) => (
                        ...
                        >
                          ...
                          <ActionPopover onProductMapping={() => toggleVendorMapping(vendor)} />
                        </tr>
                      ))
                      : !isLoading && (
                        <tr>
      ...
      >
        <AddVendorModal
          initialData={
            selectedVendor === -1 ? null : filteredVendors[selectedVendor]
          }
